...
/**
 * Fills missing features in browser if needed
 */
util.addCompatibility = function() {
	//Bind
...
/**
 * Check if the browser is running on mobile platform
 * @return True if mobile browser
 */
util.isMobileBrowser = function() {
	return /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|CriOS|Opera Mini/i.test(navigator.userAgent);
};
 ...
 */
util.mergeArrays = function(a1, a2) {
	var a = a1.concat(a2);
 ...
 */
util.sortNumberArray = function(a, b) {
	return a-b;
};
 ...
 */
util.rmDuplicatesSortedArray = function(item, pos, ary) {
 ...
 */
util.listLevels = function(tags, relations) {
	...
	//Tag level + repeat_on
		...
		var lvl1 = util.parseLevelsFloat(tags.level);
		var lvl2 = util.parseLevelsFloat(tags.repeat_on);
		
		if(lvl1 != null && lvl2 != null) {
			currentLevel = lvl1.concat(lvl2);
		}
		else if(lvl1 != null) {
			currentLevel = lvl1;
		}
	...
	//Tag level
		...
		currentLevel = util.parseLevelsFloat(tags.level);
	}
	//Tag repeat_on
	else if(tags.repeat_on != undefined) {
		currentLevel = util.parseLevelsFloat(tags.repeat_on);
	}
	//Tag min_level and max_level
	else if(tags.min_level != undefined && tags.max_level != undefined) {
		currentLevel = util.parseLevelsFloat(tags.min_level+"-"+tags.max_level);
	}
	//Tag buildingpart:verticalpassage:floorrange
	else if(tags["buildingpart:verticalpassage:floorrange"] != undefined) {
		currentLevel = util.parseLevelsFloat(tags["buildingpart:verticalpassage:floorrange"]);
	}
	//Tag building:levels (and optionally building:min_level)
	else if(tags["building:levels"] != undefined) {
		var upperLevel = Math.ceil(parseFloat(tags["building:levels"])) - 1;
		currentLevel = util.parseLevelsFloat(((tags["building:min_level"] != undefined) ? tags["building:min_level"] : "0")+"-"+upperLevel);
	}
	//Relations type=level
		...
		for(var i=0; i < relations.length; i++) {
			...
			if(rel.reltags.type == "level" && rel.reltags.level != undefined) {
				var relLevel = util.parseLevelsFloat(rel.reltags.level);
	...
	if(currentLevel != null) {
		currentLevel.sort(util.sortNumberArray);
		return currentLevel;
	} else {
 ...
 */
util.parseLevelsFloat = function(str) {
	var result = null;
	...
	if(regex1.test(str)) {
		...
		result.sort(util.sortNumberArray);
	}
 ...
 */
util.addDimensionUnit = function(v) {
 ...
 */
util.correctWebLink = function(url) {
	var result;
 ...
 */
util.distanceLevels = function(c1, l1, c2, l2) {
 ...
 */
util.filterFloat = function(value) {
	if(/^(\-|\+)?([0-9]+(\.[0-9]+)?|Infinity)$/
		.test(value))
		return Number(value);
	return NaN;
 ...
 */
util.angle360toAngle180 = function(a) {
	if(a >= 0 && a <= 360) { return (a <= 180) ? a : -(360-a); }
	else if(a > 360) { return util.angle360toAngle180(a-360); }
	else { return util.angle360toAngle180(a+360); }
};
 ...
 * @param vs The polygon
 * @return True if inside
 */
util.pointInPolygon = function(point, vs) {
 ...
 */
util.normLat = function(val) {
	return util.normAbs(val, 90);
};
 ...
 */
util.normLon = function(val) {
	return util.normAbs(val, 180);
};

/**
 * @param val The value
 * @param mod The modulo
 * @return The normalized value, between -mod and +mod
 */
util.normAbs = function(val, mod) {
